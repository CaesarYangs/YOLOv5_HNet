...
class Detect(nn.Module):
    ...
    def _make_grid(self, nx=20, ny=20, i=0, torch_1_10=check_version(torch.__version__, '1.10.0')):
        ...
        y, x = torch.arange(ny, device=d, dtype=t), torch.arange(
            nx, device=d, dtype=t)
        yv, xv = torch.meshgrid(y, x, indexing='ij') if torch_1_10 else torch.meshgrid(
            y, x)  # torch>=0.7 compatibility
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):
        ...
        if isinstance(m, (Detect, Segment)):
            ...
            def forward(x): return self.forward(
                x)[0] if isinstance(m, Segment) else self.forward(x)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):
        ...
        LOGGER.info(
            f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
