...
class Loggers():
    ...
    def __init__(self, save_dir=None, weights=None, opt=None, hyp=None, logger=None, include=LOGGERS):
        ...
        self.keys = [
            ...
            'x/lr1',
            'x/lr2']  # params
        self.best_keys = ['best/epoch', 'best/precision',
                          'best/recall', 'best/mAP_0.5', 'best/mAP_0.5:0.95']
        for k in LOGGERS:
...
class GenericLogger:
    ...
    def log_images(self, files, name='Images', epoch=0):
        ...
        files = [Path(f) for f in (files if isinstance(
            files, (tuple, list)) else [files])]  # to Path
...
def log_tensorboard_graph(tb, model, imgsz=(640, 640)):
    ...
    try:
        ...
        with warnings.catch_warnings():
            ...
            tb.add_graph(torch.jit.trace(
                de_parallel(model), im, strict=False), [])
...
def web_project_name(project):
    ...
    if not project.startswith('runs/train'):
        return project
    suffix = '-Classify' if project.endswith(
        '-cls') else '-Segment' if project.endswith('-seg') else ''
