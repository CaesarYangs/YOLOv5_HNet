...
def main():
    ...
    for epoch in range(args.resume_epoch, args.max_epochs + 1):  # 对全局的每个训练周期：
        ...
        # ======================fast training=======================
        for idx in idxs_fast_users:  # 对每个参与联邦训练的客户端。
            ...
            if epoch == 1:  # 如果是首轮训练，使用初始权重。
                os.system(
                    "python train.py --epochs {} --batch 16 --data data/fl_clients/{}.yaml --device '0' --weights weights/yolov5m.pt".format(args.local_epochs_fast, idx))

            else:  # 非首轮训练，使用聚合后的权重。
                os.system(
                    "python train.py --epochs {} --batch 16 --data data/fl_clients/{}.yaml --device '0' --weights weights/avg_ckpt_E{}.pt".format(
                        args.local_epochs_fast, idx, epoch - 1))
            ...
            exp_new_name = "runs/train/E{}_C{}_FAST".format(epoch, idx)
            os.rename(exp_name, exp_new_name)

            # os.rename(file_new_name, file_old_name)  # 练完把数据集名字改回去

        # ======================slow training=======================
        for idx in idxs_slow_users:  # 对每个参与联邦训练的客户端。
            print("client{} is training(Communication Rounds {})".format(idx, epoch))
            # file_old_name = "datasets{}".format(idx)  # 数据集改名
            # file_new_name = "datasets"
            # os.rename(file_old_name, file_new_name)
            ...
            if epoch == 1:  # 如果是首轮训练，使用初始权重。
                os.system(
                    "python train.py --epochs {} --batch 16 --data data/fl_clients/{}.yaml --device '0' --weights weights/yolov5m.pt".format(args.local_epochs_slow, idx))

            else:  # 非首轮训练，使用聚合后的权重。
                os.system(
                    "python train.py --epochs {} --batch 16 --data data/fl_clients/{}.yaml --device '0' --weights weights/avg_ckpt_E{}.pt".format(
                        args.local_epochs_slow, idx, epoch - 1))
            ...
            exp_name = "runs/train/exp"  # exp改名
            exp_new_name = "runs/train/E{}_C{}_SLOW".format(epoch, idx)
            os.rename(exp_name, exp_new_name)
        ...
        torch.save(
            Avg_ckpt, "./weights/asy_feature_test3_5/avg_ckpt_E{}.pt".format(epoch))
